...
void ktx_tex2darray::bindResources() {
...
void ktx_tex2darray::cleanupObjects() {
    ...
    delete []uboData.instance;
}
void ktx_tex2darray::loadModel() {
    geo.readFile("content/pig.obj");
    ...
    size_t vertexBufferSize = sizeof(Basic::Vertex) * geo.vertices.size();
    size_t indexBufferSize = sizeof(uint32_t) * geo.indices.size();
    geoBufferManager.createBufferWithStagingBuffer<vertex_usage>(vertexBufferSize, geo.vertices.data());
    geoBufferManager.createBufferWithStagingBuffer<index_usage>(indexBufferSize, geo.indices.data());
}

void ktx_tex2darray::loadTexture() {
    sampler = FnImage::createImageSampler(mainDevice.physicalDevice, mainDevice.logicalDevice);
    uboTexture.create("content/ktx_files/diff_array.ktx2", sampler);
}


void ktx_tex2darray::setupDescriptors() {
    // 1 ubo
    // 1 tex
...
void ktx_tex2darray::preparePipelines() {
     auto device = mainDevice.logicalDevice;
    const auto vertModule = FnPipeline::createShaderModuleFromSpvFile("shaders/texarray_vert.spv",  device);
    const auto fragModule = FnPipeline::createShaderModuleFromSpvFile("shaders/texarray_frag.spv",  device);
...
void ktx_tex2darray::recordCommandBuffer() {
 // select framebuffer
    ...
    vkCmdDrawIndexed(activatedFrameCommandBufferToSubmit, geo.indices.size(), MAX_LAYERS, 0, 0, 0);
...
void ktx_tex2darray::prepareUniformBuffers() {
    uboData.instance = new PerInstanceData[4];
    VkDeviceSize uboSize = sizeof(uboData.view) + sizeof(uboData.proj) + (MAX_LAYERS * sizeof(PerInstanceData));
    uboBuffer.createAndMapping(uboSize);


    // Array indices and model matrices are fixed
    float offset = -1.5f;
    float center = (MAX_LAYERS*offset) / 2.0f - (offset * 0.5f);
    for (uint32_t i = 0; i < MAX_LAYERS; i++) {
        uboData.instance[i].model = glm::translate(glm::mat4(1.0f), glm::vec3(i * offset - center, 0.0f, 0.0f));
        uboData.instance[i].model = glm::scale(uboData.instance[i].model, glm::vec3(0.5f));
        uboData.instance[i].arrayIndex = (float)i;        // Instance texture array index
    }
    // Update instanced part of the uniform buffer
    // only update INSTANCE part memory
    const int dataOffset = sizeof(uboData.view) + sizeof(uboData.proj) ;
    uint32_t dataSize = MAX_LAYERS * sizeof(PerInstanceData);
    memcpy((char*)uboBuffer.mapped + dataOffset, uboData.instance, dataSize);
    updateUniformBuffer();
}

void ktx_tex2darray::updateUniformBuffer() {
    auto [width, height] = simpleSwapchain.swapChainExtent;
    uboData.proj =  glm::perspective(glm::radians(45.0f), static_cast<float>(width) / static_cast<float>(height), 0.1f, 2000.0f);
    uboData.proj[1][1] *= -1;
    uboData.view =  glm::lookAt(glm::vec3(0, 0.0f, 5), glm::vec3(0.0f, 0, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));// OGL
    // only update proj view part memory
    memcpy(uboBuffer.mapped, &uboData, sizeof(uboData.view) + sizeof(uboData.proj)  );
}
