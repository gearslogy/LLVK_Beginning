...
void OpaqueScenePass::recordCommandBuffer() {
    ...
    const auto renderPassBeginInfo= FnCommand::renderPassBeginInfo(framebuffer, renderPass, pRenderer->getSwapChainExtent(),clearValues);
    const auto cmdBuf = pRenderer->getMainCommandBuffer();
    vkCmdBeginRenderPass(cmdBuf, &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
    vkCmdBindPipeline(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS , pipeline);
    vkCmdBindDescriptorSets(cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS, pRenderer->dualPipelineLayout, 0, 1, &sets[pRenderer->getCurrentFrame()], 0, nullptr);
    UT_GeometryContainer::renderPart(cmdBuf, &pRenderer->gridLoader.parts[0]);
...
void OpaqueScenePass::cleanup() {
    ...
    UT_Fn::cleanup_render_pass(device, renderPass);
    UT_Fn::cleanup_pipeline(device, pipeline);
    UT_Fn::cleanup_framebuffer(device, framebuffer);
}
...
void OpaqueScenePass::createFramebuffer() {
    ...
    auto fbCIO = FnRenderPass::framebufferCreateInfo(width,height, renderPass,attachments);
    if (vkCreateFramebuffer(device, &fbCIO, nullptr, &framebuffer) != VK_SUCCESS)
        throw std::runtime_error("failed to create main pass framebuffer!");
}


void OpaqueScenePass::createRenderPass() {
    const auto cdATM = FnRenderPass::colorAttachmentDescription(DualPassGlobal::colorAttachmentFormat);
    const auto depthATM = FnRenderPass::depthAttachmentDescription(DualPassGlobal::depthStencilAttachmentFormat);
    const auto cdATMRef = FnRenderPass::attachmentReference(0,VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
    const auto depthATMRef = FnRenderPass::attachmentReference(1,VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL);
    ...
    // create
    const auto device = pRenderer->getMainDevice().logicalDevice;
    const auto ret = vkCreateRenderPass(device, &renderPassCIO, nullptr, &renderPass);
    if(ret != VK_SUCCESS) throw std::runtime_error{"ERROR"};
}
void OpaqueScenePass::createPipeline() {
    const auto device = pRenderer->getMainDevice().logicalDevice;
    const auto vsMD = FnPipeline::createShaderModuleFromSpvFile("shaders/indirect_instance_vert.spv",  device);    //shader modules
    const auto fsMD = FnPipeline::createShaderModuleFromSpvFile("shaders/indirect_instance_frag.spv",  device);
    ...
    pso.setShaderModule(vsMD,fsMD);
    pso.setPipelineLayout(pRenderer->dualPipelineLayout);
    pso.setRenderPass(renderPass);
