...
void VmaRenderer::bindResources() {
...
void VmaRenderer::cleanupObjects() {
...
void VmaRenderer::setupDescriptors() {
    // 1 ubo
    // 1 tex
...
void VmaRenderer::preparePipelines() {
     auto device = mainDevice.logicalDevice;
...
void VmaRenderer::recordCommandBuffer() {
 // select framebuffer
...
void VmaRenderer::prepareUniformBuffers() {
    uboBuffer.createAndMapping(sizeof(uboData));
    updateUniformBuffer();
}

void VmaRenderer::updateUniformBuffer() {
