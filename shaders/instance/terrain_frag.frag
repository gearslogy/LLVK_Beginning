...
void main(){
    ...
    vec2 uv = fragTexCoord * 50 ;

    // 采样每个纹理层
    vec4 tex0 = texture(albedo_array, vec3(uv, 0));
    vec4 tex1 = texture(albedo_array, vec3(uv, 1));
    vec4 tex2 = texture(albedo_array, vec3(uv, 2));
    vec4 tex3 = texture(albedo_array, vec3(uv, 3));
